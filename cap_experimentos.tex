...
La campaña se ejecutó en una máquina virtual de propósito general, sin hardware
acelerador, cuyas especificaciones se resumen en la Tabla~\ref{tab:entorno_ab}.

\begin{table}[htbp]
\renewcommand{\arraystretch}{1.3}
\caption{Especificaciones de la máquina que ejecutó la campaña A/B de control semafórico}
\label{tab:entorno_ab}
\centering
\footnotesize
\begin{tabular}{ll}
\hline
...
\hline
Sistema operativo      & Ubuntu 24.04.4 LTS (Noble Numbat), aarch64 \\
Virtualización         & Máquina virtual KVM (virt-7.2) \\
Kernel                 & Linux 6.17.0-1020-oracle \\
Procesador             & ARM Neoverse-N1 (2 núcleos) \\
RAM                    & 11,65 GiB (13\% en uso durante la ejecución) \\
Almacenamiento         & 192,69 GiB, ext4 (22\% en uso) \\
GPU                    & RedHat Virtio 1.0 (no utilizada para cómputo) \\
Red                    & Tailscale, IP local 10.0.0.19/24 \\
\hline
\end{tabular}
\end{table}
...
\end{axis}
\end{tikzpicture}
\caption{Corridas favorables al sistema propuesto, de 10 semillas, por escenario y campaña.}
...
\end{axis}
\end{tikzpicture}
\caption{Mediana de la variación porcentual de throughput del sistema propuesto respecto
de cada plan fijo, por escenario.}
...
\end{axis}
\end{tikzpicture}
\caption{Mediana de la variación porcentual del tiempo total de sistema del sistema
propuesto respecto de cada plan fijo, por escenario.}
...
de normalidad. El test U de Mann-Whitney ordena en conjunto todos los tiempos de viaje de ambas corridas
(sin usar el emparejamiento) y contrasta si los valores de una tienden a ser
sistemáticamente mayores que los de la otra, evaluando así la separación estocástica entre
las dos distribuciones de tiempos de viaje; sirve de contraste frente al resultado pareado. El estadístico $d$ de Cohen cuantifica la magnitud del efecto en
...
\end{axis}
\end{tikzpicture}
\caption{Dispersión de las 180 comparaciones A/B: variación porcentual de throughput (eje
horizontal) contra variación porcentual de tiempo de sistema (eje vertical), coloreada
según el veredicto de la corrida. Las líneas discontinuas marcan el cero de cada eje.}
...
diez observaciones ordenadas de cada escenario.
...
sobre las diez semillas de la campaña MOPC~60~s.}
...
convención de caja y bigotes que la Figura~\ref{fig:control_box_thr}.}
...
Del conjunto de resultados se desprenden cuatro patrones consistentes.

\textbf{Ventaja creciente con la asimetría de demanda.} La ventaja del sistema propuesto
crece con la asimetría de la demanda entre aproximaciones: el escenario corredor, en el
que la calle dominante concentra el doble de demanda que las transversales, es el único
en resultar favorable en las 10 corridas de las tres campañas simultáneamente, con una
mediana de $\Delta$throughput de +207,5\% frente al plan MOPC~60~s y de +22,6\% frente al
ciclo largo~120~s. Esta magnitud refleja que un reparto de verde igualitario resulta
especialmente ineficiente cuando la demanda real está lejos de ser simétrica, condición
que el control adaptativo del sistema propuesto explota de forma directa.

\textbf{Tipo de ventaja según el plan de referencia.} El tipo de ventaja que obtiene el
sistema propuesto depende del ciclo del plan fijo contra el que se compara. Frente al
plan MOPC de 60~s, cuyo ciclo corto y alto tiempo perdido relativo lo hacen más
vulnerable a saturarse, la ventaja proviene principalmente de evitar el colapso: el
sistema propuesto sostiene mayor throughput y menor tiempo de sistema incluso en
escenarios de demanda alta y saturada. En cambio, frente a los planes de 90~s y 120~s,
cuyo tiempo perdido por ciclo es menor y cuya capacidad de servicio está mejor
dimensionada, la ventaja del sistema propuesto se concentra en los regímenes de demanda
baja y media y en el escenario de asimetría marcada (corredor), mientras que en demanda
alta y saturada las medianas de $\Delta$sys se vuelven negativas (-7,2\% y -5,2\% frente a
netconvert~90~s; -8,8\% y -7,3\% frente al ciclo largo~120~s), señal de que el margen de
mejora disponible se reduce cuando el plan de referencia ya opera cerca de su capacidad.

\textbf{Límite del sistema: colapsos en demanda alta.} El escenario demanda\_alta es el
...
\textbf{Ninguna semilla es uniformemente desfavorable.} En las matrices de veredicto de la
Figura~\ref{fig:control_grillas}, el patrón dominante es horizontal: filas completas o
casi completas de un mismo color (corredor en verde en las tres matrices; demanda\_alta
con una mezcla persistente de rojo en las tres matrices) muestran que el escenario de
demanda determina el resultado con mucha mayor fuerza que la semilla concreta, mientras
que dentro de una misma fila el color rara vez cambia de forma sistemática por columna:
no existe una semilla que sea uniformemente desfavorable a través de todos los
escenarios.
...
Los tres frentes evaluados confirman la viabilidad del sistema para entornos con
presupuesto limitado, como ciudades intermedias de la región: traffic-sync escala
linealmente dentro de la memoria disponible en hardware embebido de bajo costo, y
traffic-storage ofrece latencias de subida y descarga compatibles con su modelo de
reporte periódico sobre datos agregados, no de control en tiempo estricto. El cuello de
botella identificado es el tiempo de confirmación en la red BlockDAG, mitigable mediante
la gestión de nonces y la precarga del estado de gas sin modificar la infraestructura de
red. El Capítulo~\ref{chp:conclusion} retoma estas cifras junto con las de la campaña A/B
en la síntesis final del trabajo.

\subsection{Desafíos Identificados y Limitaciones}
\label{subsec:discusion_limitaciones}

Durante la ejecución de las pruebas se identificaron dos desafíos técnicos relevantes
para un despliegue de producción: la dependencia de Pinata como nodo IPFS gestionado por
terceros, y la variabilidad en los tiempos de minado de la red BlockDAG pública de
pruebas. El Capítulo~\ref{chp:conclusion} desarrolla estas limitaciones junto con las del
control semafórico y las líneas de trabajo futuro que se desprenden de ellas.

\subsection{Hallazgos del control semafórico}
\label{subsec:discusion_control}

La campaña A/B de la Sección~\ref{sec:experimentos_control_ab} mide el efecto del sistema
sobre la circulación vehicular bajo una metodología diseñada para no premiar al sistema
propuesto por artefactos de medición: sin la guardia de throughput, el sesgo de
supervivencia habría inflado la tasa de éxito reportada, en particular en los escenarios
de demanda alta y saturada, donde ambos lados de la comparación operan cerca del límite
de capacidad de la red. El límite observado en demanda alta simétrica (5 de 10 corridas
favorables) no es una falla del control adaptativo sino el reflejo de que, cuando no hay
asimetría de demanda que explotar, un plan fijo bien dimensionado se acerca a una
asignación de verde casi óptima y el costo de conmutación de Max-Pressure erosiona parte
del margen disponible. El Capítulo~\ref{chp:conclusion} retoma este límite, junto con los
colapsos parciales autoinducidos identificados en esa misma celda
(Sección~\ref{subsec:control_patrones}), en la discusión de limitaciones y trabajo
futuro.
