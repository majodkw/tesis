...
La Tabla~\ref{tab:blockchain_vs_blockdag} sintetiza las diferencias estructurales entre ambas arquitecturas. La comparación considera protocolos representativos de cada paradigma: Nakamoto consensus implementado en Bitcoin y Ethereum para blockchains lineales \cite{nakamoto2008bitcoin,buterin2014ethereum}, y PHANTOM/GHOSTDAG como generalización del consenso distribuido en estructuras DAG \cite{sompolinsky2021phantom}.

\begin{table}[htbp]
\centering
\caption{Diferencias estructurales entre una blockchain lineal y un BlockDAG con consenso GHOSTDAG \cite{sompolinsky2021phantom,zhang2025sok}. La comparación es cualitativa: las cifras de rendimiento medidas en este trabajo se presentan en la Tabla~\ref{tab:comparativa_blockchain}.}
\label{tab:blockchain_vs_blockdag}
\renewcommand{\arraystretch}{1.3}
\footnotesize
\begin{tabular}{@{}p{2.9cm}p{5.6cm}p{5.6cm}@{}}
\toprule
\textbf{Característica} & \textbf{Blockchain lineal} & \textbf{BlockDAG (GHOSTDAG)} \\
\midrule
Topología
  & Cada bloque referencia un único padre
  & Cada bloque referencia varios padres \\
Bloques concurrentes
  & Se descartan como huérfanos
  & Se integran al grafo y cuentan para el consenso \\
Ordenamiento
  & Total, inducido por la propia cadena
  & Parcial en el grafo; el algoritmo de consenso lo completa a un orden total \\
Minería
  & Competitiva: el bloque que llega segundo se pierde
  & Cooperativa: todo bloque válido aporta trabajo al consenso \\
Límite de rendimiento
  & El protocolo frena la tasa de bloques para evitar huérfanos
  & El ancho de banda de la red es el límite práctico \\
Confirmación
  & Probabilística; exige esperar varios bloques sucesivos
  & Probabilística; la espera se acorta al integrar bloques paralelos \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \texttt{Optimization} (valor~1): resultado del ciclo difuso-PSO (largo de ciclo óptimo por clúster, usado aguas abajo como techo blando de referencia por el controlador táctico local).
\end{itemize}
