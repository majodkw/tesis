...
Bloques huérfanos & Descartados (seguridad desperdiciada)\cite{sompolinsky2021phantom} & Integrados como tíos/padres (seguridad acumulativa)\cite{sompolinsky2021phantom,kaspa_whitepaper} \\
Ordenamiento & Total (intrínseco a la cadena) & Parcial (topológico) $\rightarrow$ Total (vía algoritmo)\cite{sompolinsky2021phantom} \\
Escalabilidad & Limitada por latencia de propagación\cite{sompolinsky2021phantom} & Alta (limitada por ancho de banda físico)\cite{sompolinsky2021phantom,zhang2025sok} \\
Confirmación & Probabilística lenta (e.g., 6 bloques) & Probabilística rápida (segundos)\cite{zhang2025sok,kaspa_whitepaper} \\
Minería & Competitiva (winner-takes-all)\cite{sompolinsky2021phantom} & Cooperativa (pay-per-share implícito)\cite{sompolinsky2021phantom} \\
Throughput típico & 7-30 TPS\cite{nakamoto2008bitcoin,buterin2014ethereum} & 100+ TPS (depende de implementación)\cite{kaspa_whitepaper,zhang2025sok} \\
Latencia de bloque & 10 min (Bitcoin)\cite{bitcoin_blocktime}, 12 s (Ethereum)\cite{ethereum_blocks} & Sub-segundo a segundos\cite{kaspa_whitepaper,zhang2025sok} \\
\hline
\end{tabular}
\end{table}
