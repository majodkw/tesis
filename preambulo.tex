...
\usepackage{pgfplots} % For data plots and charts
\pgfplotsset{compat=1.18}
\usepackage{pgfplotstable} % For table-based plots
\usepackage{colortbl}
\usepackage{xcolor} % (Probablemente ya lo tengas)
\usepgfplotslibrary{statistics} % For box plots if needed
...
% 'para cada X hacer' en vez de 'para cada X repetir'
\algnewcommand\algorithmicforeachdo{\textbf{hacer}}
\algdef{S}[FOR]{ForEach}[1]{\algorithmicforeach\ #1\ \algorithmicforeachdo}
...
% los epigrafes decian 'Cuadro' mientras toda la prosa referencia 'Tabla'
